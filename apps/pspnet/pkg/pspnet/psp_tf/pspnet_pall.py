...
class PSPNet(object):
    ...
    def model_pall(self,input_data):
      self.queueLock.acquire()
      ret=self.model.predict(input_data)[0]
    ...
    def predict(self, img, flip_evaluation):
        ...
        regular_prediction = self.model_pall(input_data)
        if flip_evaluation:
...
def predict_sliding(full_image, net, flip_evaluation):
    ...
    def local_func(workQueue,row,col,tile_size,stride,full_image,tile_counter):
            x1 = int(col * stride)
            y1 = int(row * stride)
            ...
            workQueue.put((padded_img, flip_evaluation,(x1,x2,y1,y2),tile_counter,img.shape))    
            
    for row in range(tile_rows):
      for col in range(tile_cols):    
          tile_counter += 1
          _thread.start_new_thread(local_func,(workQueue,row,col,tile_size,stride,full_image,tile_counter))
    idx=0
    import time
    while idx<tile_rows*tile_cols:
            print("wait for ({0}/{1})...".format( idx,tile_rows*tile_cols))
            queueLock.acquire()
            if not workQueue.empty():
                (padded_img, flip_evaluation,(x1,x2,y1,y2),tile_counter,img_shape)=workQueue.get()
                queueLock.release()
                padded_prediction=net.predict(padded_img, flip_evaluation)
                prediction = padded_prediction[0:img_shape[0], 0:img_shape[1], :]
