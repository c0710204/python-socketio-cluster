...
def predict_sliding(full_image, net, flip_evaluation):
    ...
    tile_counter = 0
    for row in tqdm.trange(tile_rows):
        for col in tqdm.trange(tile_cols):
            x1 = int(col * stride)
            y1 = int(row * stride)
...
if __name__ == "__main__":
    ...
    with sess.as_default():
        ...
        for fpath in tqdm.tqdm(onlyfiles):
          img = misc.imread(join(args.input_path,fpath))
          ...
          filename, ext = splitext(join(args.output_path,fpath))
          #misc.imsave(filename + "_seg" + ext, colored_class_image)
          #misc.imsave(filename + "_probs" + ext, pm)
          misc.imsave(filename + "_seg_blended" + ext, alpha_blended)
          l=[np.count_nonzero(img_class == i) for i in range(150)]
