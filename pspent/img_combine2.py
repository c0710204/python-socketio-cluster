...
def img_combine2(args):
  ...
  def funchandler(inp):
    if len(inp)==7:
      ...
      return np.load("{0}_-_{5}_-_{1}_-_{2}_-_{3}_-_{4}_-_.npy".format(filename,inp[2],inp[3],inp[4],inp[5],inp[6]))
    
  return img_combine_func2.predict_multi_scale(funchandler, img_shape, pspnet, EVALUATION_SCALES, args.sliding, args.flip,args)
