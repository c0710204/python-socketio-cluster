...
def sshdownload(data):
    ssht = sshtunnel.SSHTunnelForwarder(data['proxy']['host'],ssh_username=data['proxy']['username'],ssh_password=data['proxy']['password'],remote_bind_address=(data['ssh']['host'], data['ssh']['port']))
    ssht.start()
    print(ssht.local_bind_port)
    sftp=pysftp.Connection("127.0.0.1", username=data['ssh']['username'], password=data['ssh']['password'],port=ssht.local_bind_port, cnopts=cnopts)
...
class Pspnet_namespace(BaseNamespace):
  ...
  def on_request(self,*args):
    ...
    socketIO=SocketIO('localhost', ssht2.local_bind_port, LoggingNamespace)
    ...
    class_scores=img_combine2.img_combine2(namedtuple('Struct', args_d.keys())(*args_d.values()))
    print("blended...")
    ...
    colored_class_image = utils.color_class_image(class_image, args_d['model'])
