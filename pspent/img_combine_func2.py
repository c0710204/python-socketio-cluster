...
def ndimage_zoom_parallel_2(list_all):
    ...
    def funchandler(inp):
      ...
      fpath="{0}_-_{5}_-_{1}_-_{2}_-_{3}_-_{4}_-_.npy".format(filename,inp[2],inp[3],inp[4],inp[5],inp[6])
      ret=np.load(fpath)
      os.remove(fpath)
      return ret
...
def predict_multi_scale(funchandler,full_image_shape, net, scales, sliding_evaluation, flip_evaluation,args):
  ...
  with tqdm(scales) as pbar:
    ...
    for scale in pbar:
        ...
        probs_inp.append((args.input_path2,scale,h_ori, w_ori,flip_evaluation,net,n,full_image_shape,sliding_evaluation))
